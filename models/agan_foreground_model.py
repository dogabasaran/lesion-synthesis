...
class AttentionGANForegroundModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'D_C', 'cycle_B', 'idt_B']
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A, _, _, _, _, _, _, _, _, _, _, _, _, \
            _, _, _, _, _, _, _, _, _, _, \
        ...
        # GAN loss D_A(G_A(A))
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B) + self.netD_C(self.fake_C), True)        # here we feed in the discriminator of lesions
        # GAN loss D_B(G_B(B))
