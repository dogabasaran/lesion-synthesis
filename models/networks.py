...
class ResnetGenerator_our(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=9):
        ...
        self.deconv3_content = nn.Conv2d(ngf, 9, 7, 1, 0)
    ...
    # forward method                    ~NOTE: CURRENTLY GRAYSCALE MODE.
    def forward(self, input):
        ...
        attention1 = attention1_.repeat(1, 1, 1, 1)
        attention2 = attention2_.repeat(1, 1, 1, 1)
        attention3 = attention3_.repeat(1, 1, 1, 1)
        attention4 = attention4_.repeat(1, 1, 1, 1)
        attention5 = attention5_.repeat(1, 1, 1, 1)
        attention6 = attention6_.repeat(1, 1, 1, 1)
        attention7 = attention7_.repeat(1, 1, 1, 1)
        attention8 = attention8_.repeat(1, 1, 1, 1)
        attention9 = attention9_.repeat(1, 1, 1, 1)
        attention10 = attention10_.repeat(1, 1, 1, 1)
        ...
        foreground = output1 + output2 + output3 + output4 + output5 + output6 + output7 + output8 + output9
        background = output10
        o=output1 + output2 + output3 + output4 + output5 + output6 + output7 + output8 + output9 + output10
        return o, foreground, background, output1, output2, output3, output4, output5, output6, output7, output8, output9, output10, attention1,attention2,attention3, attention4, attention5, attention6, attention7, attention8,attention9,attention10, image1, image2,image3,image4,image5,image6,image7,image8,image9
...
class resnet_block(nn.Module):
    ...
    def forward(self, input):
        drop = True
        dropout_level= .50 #probability of an element to be zeroed
        x = F.pad(input, (self.padding, self.padding, self.padding, self.padding), 'reflect')
        x = F.relu(self.conv1_norm(self.conv1(x)))
        if drop:
            # print('dropping')
            droppingout = nn.Dropout(dropout_level)
            x = droppingout(x)
